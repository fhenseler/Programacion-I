        ...
        for(j = 0; j < tamPP; j++)
        {
            if(lista[i].estado == 1 && lista[i].idProgramador == progproy[j].idProgramador)
            {
                for(k = 0; k < tamY; k++)
                {
                    ...
                    {
                        obtenerDescripcion(descripcionCat, categorias, tamC, lista[j].idCategoria);
                        suma = obtenerSumaACobrar(suma, categorias, tamC, lista[k].idCategoria);
                        cantHoras = obtenerCantHoras(cantHoras, lista, tamL, lista[k].cantidadHoras);
