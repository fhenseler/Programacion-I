...
void mostrarProgramador(eProgramador programador[], int tamL, eCategoria categoria[], int tamC)
{
    ...
    for(int i = 0; i < tamL; i++)
    {
        if(programador[i].estado == 1)
        {
            ...
            printf("\n%d      %s       %s        %s          %d", programador[i].idProgramador, programador[i].nombre, programador[i].apellido, descripcionCat, programador[i].totalACobrar);
        }
    }
}
...
void mostrarProgramadores(eProgramador lista[], int tamL , eCategoria categorias[], int tamC)
{
    ...
    printf("\n\n*ID*   *NOMBRE*   *APELLIDO*   *CATEGORIA*    *TOTAL A COBRAR*\t\n");
    for(int i = 0; i < tamL; i++)
    {
...
void mostrarProyectoCant(eProyecto proyecto[], int tamY, eProgramador lista[], int tamL)
{
    for(int i = 0; i < tamY; i++)
    {
        if(proyecto[i].estado == 1)
        {
            printf("\n%d\t %s\t   %d\t   %d\t %d\t   %d\t", proyecto[i].idProyecto, proyecto[i].nombre, proyecto[i].cantidadProg1, proyecto[i].cantidadProg2, proyecto[i].cantidadProg3, proyecto[i].totalAPagar);
        }
    }
...
void mostrarProyectosCant(eProyecto proyectos[], int tamY, eProgramador lista[], int tamL)
{
    int flag = 0;
    printf("\n[LISTA DE PROYECTOS]\n");
    printf("\n\n*ID*\t*NOMBRE*\t*C1*\t*C2*\t*C3*\t*TOTAL A PAGAR*\t\n");
    for(int i = 0; i < tamY; i++)
    {
        if(proyectos[i].estado == 1 && flag == 0)
        {
...
void asignarProgramador(eProgramador lista[], int tamL, eProyecto proyectos[], int tamY, eProg_Proy progproy[], int tamPP, eCategoria categorias[], int tamC)
{
        ...
        indiceLibre = findEmptyPlace2(progproy, tamPP);
                ...
                for(int j = 0; j < tamY; j++)
                {
                    if(proyectos[j].idProyecto == aux2)
                    {
                        for(int k = 0; k < tamPP; k++)
                        {
                                ...
                                {
//                                    progproy[k].idProgramadorProyecto++;

//                                    k = buscarPorID2(progproy, progproy[k].idProgramadorProyecto, tamPP);
//
//                                    if(k != -1 && progproy[k].estado == 1)
//                                    {
//                                        if(progproy[k].idProgramador == aux && progproy[k].idProyecto == aux2)
//                                        {
//                                            printf("\n\nERROR! Este programador ya fue asignado a este proyecto.\n\n");
//                                            break;
//                                        }
//                                    }
                                   ...
                                   if(lista[i].idCategoria == 3)
                                   {
                                       ...
                                       printf("El programador %d fue asignado al proyecto %d. Debera trabajar %d horas.", progproy[k].idProgramador, progproy[k].idProyecto, progproy[k].cantidadHoras);
                                       progproy[k].estado = 1;
                                       break;
                                }
...
int obtenerCantHoras(int cant, eProg_Proy progproy[], int tam, int cantHoras)
{
    int i;
    for(i=0; i< tam; i++)
    {
        if(progproy[i].cantidadHoras == cantHoras)
        {
            cant = progproy[i].cantidadHoras;
            return cant;
        }
    }
            return 0;
            ...
            if(lista[i].estado == 1 && lista[i].idProgramador == progproy[j].idProgramador)
            {
                for(k = 0; k < tamY; k++)
                {
                    if(proyectos[k].estado == 1 && proyectos[k].idProyecto == progproy[j].idProyecto)
                    {
                        ...
                        cantHoras = obtenerCantHoras(cantHoras, progproy, tamPP, progproy[j].cantidadHoras);
                        proyectos[k].totalAPagar += suma * cantHoras;
                        lista[i].totalACobrar += suma * cantHoras;
...
void mostrarProyectosProg(eProgramador lista[], int tamL, eProyecto proyectos[], int tamY, eProg_Proy progproy[], int tamPP, eCategoria categorias[], int tamC)
{
    ...
    printf("\n\nID PROGRAMADOR    NOMBRE   APELLIDO     PROYECTO     CANT HORAS    TOTAL A COBRAR");
    for(i = 0; i < tamL; i++)
    {
            ...
            for(j = 0; j < tamY; j++)
            {
                if(lista[i].estado == 1 && lista[i].idProgramador == progproy[j].idProgramador)
                {
                    for(k = 0; k < tamY; k++)
                    {
                        ...
                        {
                            printf("\n%d                   %s     %s     %s    %d    %d", lista[i].idProgramador, lista[i].nombre, lista[i].apellido, proyectos[k].nombre, progproy[j].cantidadHoras, lista[i].totalACobrar);
                        }
                    }
                }
            }
...
// * \return index El índice si fue encontrado, si no -1
// */
//int buscarPorID2(eProg_Proy progproy[], int idAux, int tam)
//{
//        int index = -1;
//
//        for(int i = 0; i < tam; i++)
//        {
//            if(progproy[i].idProgramadorProyecto == idAux)
//            {
//                index = i;
//                break;
//            }
 ...
 * \return index El primer lugar disponible
 */
int findEmptyPlace2(eProg_Proy progproy[], int arrayLength)
{
        int index = -1;

        for(int i = 0; i < arrayLength; i++)
        {
            if(progproy[i].estado == 0)
            {
                index = i;
                break;
            }
