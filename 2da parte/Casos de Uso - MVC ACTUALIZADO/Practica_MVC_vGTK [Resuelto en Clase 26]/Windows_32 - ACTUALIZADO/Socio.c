...
Socio* soc_new(char* nombre, char* apellido, char* dni, int id, int estado, int contServicios)
{
    ...
    soc_setContServicios(nuevoSocio, contServicios);
...
Socio* soc_findByDni(ArrayList* pArraySocio, char* dni)
{
    ...
    if(pArraySocio != NULL)
    {
        ...
        {
            auxSocio = al_get(pArraySocio, i);
            if(strcmp(soc_getDni(auxSocio), dni)==0)
            {
                retorno = auxSocio;
            }
        }
...
int soc_setContServicios(Socio* this,int cont)
{
...
int soc_filterSocio(void* item)
{
    ...
    Socio* auxSocio = (Socio*)item;

    if(auxSocio->id > 10 && auxSocio->id < 20)
    {
        retorno = 1;
    }
